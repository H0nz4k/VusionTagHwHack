...
/*
 * EXP-032 / v0.4k — same encoding as v0.4j, native 19 B/row (152/8).
 * Pervasive 2.66": V=296, H=152, size = 296*(152/8) = 5624.
 * EXP-031 37 B/row smeared BLACK/WHITE/RED into diagonals (muddy red).
 *
 *   WHITE = plane10 0, plane13 0
 *   BLACK = plane10 1, plane13 0
 *   RED   = plane10 0, plane13 1
 *
 * Row = 19 bytes. Regions:
 *   col 0..5   BLACK
 *   col 6..12  WHITE
 *   col 13..18 RED
 * First 32 rows, col 0..8 forced BLACK (corner marker into white).
 */
...
void main(void)
{
    ...
    uart_crlf();
    uart_puts("OpenVusion GU140 EXP-032 v0.4k B19");
    uart_crlf();
