...
/*
 * EXP-029 / v0.4h — clone of working v0.4g plus DCDC 0x04/0x00 and
 * refresh 0x12/0x00. No extra helpers. ~15 s hold = existing HB loop
 * (delay_crude per beat). COG_powerOff left to TAG OFF after capture.
 * White: 0x10=0xFF, 0x13=0x00 (Pervasive BWR demo background).
 */
...
void main(void)
{
    ...
    uart_crlf();
    uart_puts("OpenVusion GU140 EXP-029 v0.4h REF");
    uart_crlf();
