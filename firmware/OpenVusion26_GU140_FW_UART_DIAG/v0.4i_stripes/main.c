...
/*
 * EXP-030 / v0.4i — clone of working v0.4h with deterministic stripes.
 * Pervasive KB first frame: 1=black, 0=white. Second frame 0x13 = 0x00.
 * 296 px / 8 = 37 bytes/row. Alternate 0x00/0xFF per byte, reset each row
 * → 8 px vertical stripes. Streamed, no framebuffer in RAM.
 */
...
void main(void)
{
    ...
    uart_crlf();
    uart_puts("OpenVusion GU140 EXP-030 v0.4i STR");
    uart_crlf();
