...
/*
 * EXP-065 / v0.11i — wait with I2C idle so RF can write 64 B, then dump F8–FB.
 * One change vs v0.11h: no SRAM poll during a bounded RF window.
 */
...
void main(void)
{
    ...
    uart_crlf();
    uart_puts("OpenVusion GU140 EXP-065 v0.11i NFC-D SRAMWAIT");
    uart_crlf();
    ...
    uart_puts("READY");
    uart_crlf();
    /* I2C idle window for RF WRITE of 64 B. delay_crude is ~1–2 s. */
    for (n = 0u; n < 8u; ++n) {
        delay_crude();
    }
    uart_puts("TRANSFER");
