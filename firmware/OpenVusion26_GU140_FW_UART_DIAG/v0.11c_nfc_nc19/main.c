...
/*
 * EXP-059 / v0.11c — same I2C session write as EXP-058, data byte 0x19 not 0x59.
 * 0x19 = current RF NC_REG without PTHRU. One variable vs 058.
 * No P1_0. No EEPROM. No EPD. P2_3/P2_4 untouched. --nooverlay.
 */

#define NC_REG_BYTE  0x19u
...
void main(void)
{
    ...
    uart_crlf();
    uart_puts("OpenVusion GU140 EXP-059 v0.11c NFC-D NC19");
    uart_crlf();
    ...
    ack_nc = i2c_write(NC_REG_BYTE);
    i2c_stop();

    uart_puts("NC19 ACKAA=");
    uart_hex8(ack_aa);
    uart_puts(" ACKFE=");
    uart_hex8(ack_fe);
    uart_puts(" ACKNC=");
