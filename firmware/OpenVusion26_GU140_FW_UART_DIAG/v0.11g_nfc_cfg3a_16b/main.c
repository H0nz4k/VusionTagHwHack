...
/*
 * EXP-063 / v0.11g — I2C read-modify-write of NTAG I2C Plus 1K config
 * block 0x3A as a full 16-byte block (NXP NT3H2111: I2C is always 16 B).
 *
 * One change vs v0.11f: 16-byte RMW, not a 4-byte write of 1B 00 40 48.
 * SRAM_MIRROR_BLOCK is an I2C block (01h..34h on 1K), not an NFC page.
 * NFC page 0x40 = I2C block 0x10. Keep REG_LOCK / WDT / CLOCK_STR / RFU.
 *
 * TWN4 must be off during the EEPROM write. Config EEPROM is visible on
 * I2C 0x3A immediately after the write cycle; session NC_REG follows POR.
 *
 * No P1_0. No EPD. P2_3/P2_4 untouched. --nooverlay.
 */
...
void main(void)
{
    ...
    uart_crlf();
    uart_puts("OpenVusion GU140 EXP-063 v0.11g NFC-D CFG3A16");
    uart_crlf();
    ...
    while (1) {
        ...
        uart_hex8(n);
        uart_puts(" F8");
        (void)i2c_read_block(SRAM_BLK, blk);
        uart_crlf();
        uart_putc('E');
        uart_hex8(n);
        uart_puts(" 10");
