#!/usr/bin/env bash
# EXP-031: B/W/R calibration image + DCDC + 0x12. Isolated. TAG OFF after.
set -euo pipefail
ROOT=/home/hw/OpenVusion26_FW/OpenVusion26_GU140_FW_UART_DIAG
RELAYS=/home/hw/bin/ov26-relays.sh
HEX="$ROOT/build/v0.4j_bwr_cal.hex"
UART="/dev/serial/by-id/usb-Silicon_Labs_CP2102_USB_to_UART_Bridge_Controller_0001-if00-port0"
IDENT=/tmp/ov26_exp031_ident.txt
FLASHLOG=/tmp/ov26_exp031_flash.txt
POR=/tmp/ov26_exp031_por.bin
...
b = Path("/tmp/ov26_exp031_por.bin").read_bytes()
t = b.decode("ascii", "replace")
print(t[:4000])
print("---TAIL---")
print(t[-800:])
print("BYTES", len(b), "K", t.count("EXP-031"), "DONE", t.count("DONE"))
print("HB", t.count("HB "), "BUSY0", t.count("BUSY=0"), "BUSY1", t.count("BUSY=1"))
ok = t.count("EXP-031")==1 and t.count("DONE")==1 and "REF=01" in t and "N10=15F8" in t and "N13=15F8" in t
