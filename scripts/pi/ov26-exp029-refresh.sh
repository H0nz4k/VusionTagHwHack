#!/usr/bin/env bash
# EXP-029: DCDC + 0x12 refresh, ~17s hold. Isolated. TAG OFF after (image should persist).
set -euo pipefail
ROOT=/home/hw/OpenVusion26_FW/OpenVusion26_GU140_FW_UART_DIAG
RELAYS=/home/hw/bin/ov26-relays.sh
HEX="$ROOT/build/v0.4h_refresh.hex"
UART="/dev/serial/by-id/usb-Silicon_Labs_CP2102_USB_to_UART_Bridge_Controller_0001-if00-port0"
IDENT=/tmp/ov26_exp029_ident.txt
FLASHLOG=/tmp/ov26_exp029_flash.txt
POR=/tmp/ov26_exp029_por.bin
...
b = Path("/tmp/ov26_exp029_por.bin").read_bytes()
t = b.decode("ascii", "replace")
print(t[:4000])
print("---TAIL---")
print(t[-800:])
print("BYTES", len(b), "K", t.count("EXP-029"), "DONE", t.count("DONE"))
hb = t.count("HB ")
busy0 = t.count("BUSY=0")
busy1 = t.count("BUSY=1")
print("HB", hb, "BUSY0", busy0, "BUSY1", busy1)
ok = t.count("EXP-029")==1 and t.count("DONE")==1 and "REF=01" in t and "N10=15F8" in t and "N13=15F8" in t
