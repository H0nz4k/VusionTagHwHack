#!/usr/bin/env bash
# EXP-022: Phase 1 passive BUSY. Isolated runtime. TAG OFF after capture.
set -euo pipefail
ROOT=/home/hw/OpenVusion26_FW/OpenVusion26_GU140_FW_UART_DIAG
RELAYS=/home/hw/bin/ov26-relays.sh
HEX="$ROOT/build/v0.4a_busy_passive.hex"
UART="/dev/serial/by-id/usb-Silicon_Labs_CP2102_USB_to_UART_Bridge_Controller_0001-if00-port0"
IDENT=/tmp/ov26_exp022_ident.txt
FLASHLOG=/tmp/ov26_exp022_flash.txt
POR=/tmp/ov26_exp022_por.bin
...
b = Path("/tmp/ov26_exp022_por.bin").read_bytes()
t = b.decode("ascii", "replace")
print(t[:2000])
print("BYTES", len(b), "K", t.count("EXP-022"))
print("UART", "PASS" if t.count("EXP-022") == 1 else "FAIL")
PY
echo DONE
