#!/usr/bin/env bash
# DEV only. Isolated flash v0.10e, capture boot UART, leave TAG ON.
set -euo pipefail
...
sudo cc-tool -v read -e -w "$HEX" | tee /tmp/ov26_exp054_flash.txt
"$RELAYS" dbg-off
"$RELAYS" usb-off
"$RELAYS" tag-off
sleep 2
...
sleep 0.5
echo FLASH_OK
# First UART after USB cycle is often garbage. Power-cycle again for recap.
"$RELAYS" idle
sleep 0.3
timeout 1 cat "$UART" >/dev/null 2>&1 || true
sleep 0.4
...
print(t[:2500])
print("BYTES", len(b), "SHOW3", t.count("SHOW3"), "ARMED", t.count("ARMED"))
PY
echo "TAG ON. Menu: /home/hw/bin/ov26-nfc-show.sh"
