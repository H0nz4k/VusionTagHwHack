#!/usr/bin/env bash
# DEV only. Isolated flash v0.10f SHOW4 (same hex name v0.10e_nfc_show3), recap UART, TAG ON.
set -euo pipefail
...
sudo cc-tool -t | tee /tmp/ov26_exp055_ident.txt | grep -q CC2510 || { echo "not CC2510"; "$RELAYS" idle; exit 3; }
sudo cc-tool -v read -e -w "$HEX" | tee /tmp/ov26_exp055_flash.txt
"$RELAYS" dbg-off
"$RELAYS" usb-off
"$RELAYS" tag-off
sleep 2
...
print(t[:2500])
print("BYTES", len(b), "SHOW4", t.count("SHOW4"), "v0.10f", t.count("v0.10f"), "ARMED", t.count("ARMED"))
PY
echo "TAG ON. Menu: /home/hw/bin/ov26-nfc-show.sh  (4 = smazat)"
