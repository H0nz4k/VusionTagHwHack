#!/usr/bin/env bash
# EXP-023: Phase 2 P0_0 PWR only. Isolated runtime. TAG OFF after.
set -euo pipefail
ROOT=/home/hw/OpenVusion26_FW/OpenVusion26_GU140_FW_UART_DIAG
RELAYS=/home/hw/bin/ov26-relays.sh
HEX="$ROOT/build/v0.4b_pwr_only.hex"
UART="/dev/serial/by-id/usb-Silicon_Labs_CP2102_USB_to_UART_Bridge_Controller_0001-if00-port0"
IDENT=/tmp/ov26_exp023_ident.txt
FLASHLOG=/tmp/ov26_exp023_flash.txt
POR=/tmp/ov26_exp023_por.bin
...
b = Path("/tmp/ov26_exp023_por.bin").read_bytes()
t = b.decode("ascii", "replace")
print(t[:2500])
print("BYTES", len(b), "K", t.count("EXP-023"), "DONE", t.count("DONE"))
print("UART", "PASS" if t.count("EXP-023") == 1 else "FAIL")
PY
echo DONE
