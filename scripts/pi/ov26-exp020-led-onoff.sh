...
HEX="$ROOT/build/v0.3j_led_onoff.hex"
UART="/dev/serial/by-id/usb-Silicon_Labs_CP2102_USB_to_UART_Bridge_Controller_0001-if00-port0"
IDENT=/tmp/ov26_exp020_ident.txt
FLASHLOG=/tmp/ov26_exp020_flash.txt
POR=/tmp/ov26_exp020_por.bin
LEAVE_ON=0
...
b = Path("/tmp/ov26_exp020_por.bin").read_bytes()
t = b.decode("ascii", "replace")
print(t[:700])
print("BYTES", len(b), "J", t.count("v0.3j"))
print("UART", "PASS" if t.count("v0.3j") == 1 else "FAIL")
PY
if python3 -c "from pathlib import Path; raise SystemExit(0 if Path('/tmp/ov26_exp020_por.bin').read_bytes().decode('ascii','replace').count('v0.3j')==1 else 1)"; then
    LEAVE_ON=1
    "$RELAYS" dbg-off
    "$RELAYS" usb-off
    "$RELAYS" tag-on
