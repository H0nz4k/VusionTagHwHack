#!/usr/bin/env bash
# EXP-024: Phase 3 P2_0 H-L-H after PWR ON. Isolated. TAG OFF after.
set -euo pipefail
ROOT=/home/hw/OpenVusion26_FW/OpenVusion26_GU140_FW_UART_DIAG
RELAYS=/home/hw/bin/ov26-relays.sh
HEX="$ROOT/build/v0.4c_reset_hlh.hex"
UART="/dev/serial/by-id/usb-Silicon_Labs_CP2102_USB_to_UART_Bridge_Controller_0001-if00-port0"
IDENT=/tmp/ov26_exp024_ident.txt
FLASHLOG=/tmp/ov26_exp024_flash.txt
POR=/tmp/ov26_exp024_por.bin
...
b = Path("/tmp/ov26_exp024_por.bin").read_bytes()
t = b.decode("ascii", "replace")
print(t[:2800])
print("BYTES", len(b), "K", t.count("EXP-024"), "DONE", t.count("DONE"))
print("UART", "PASS" if t.count("EXP-024") == 1 else "FAIL")
PY
echo DONE
